...
public class ActionManager {
	...
	public ActionManager (GameManager aGameManager) {
		String tFullTitle;
		
		gameManager = aGameManager;
		tFullTitle = gameManager.createFrameTitle ("Action Report");
		actions = new LinkedList<Action> ();
		actionReportFrame = new ActionReportFrame (tFullTitle, gameManager.getGameName ());
	...
	public void actionReport () {
		...
		for (Action tAction: actions) {
			System.out.println (tAction.getActionReport (gameManager));
		}
		System.out.println ("<---------------------------");
	}
	...
	private void appendToReportFrame (Action aAction) {
		actionReportFrame.append ("\n\n" + aAction.getActionReport (gameManager));
	}
	...
	public void addAction (Action aAction) {
		...
		if (gameManager.isNetworkGame () && gameManager.shouldNotifyNetwork ()) {
	...
	public boolean undoLastAction (GameManager aGameManager) {
		...
		tLastActionUndone = tLastAction.undoAction (aGameManager);
		if (tLastActionUndone) {
			removeLastAction ();
			if (tLastAction.getChainToPrevious ()) {
				tLastActionUndone = undoLastAction (aGameManager);
			}
		}
		aGameManager.updateAllFrames ();
	...
	public void appendToJGameClient (Action aAction) {
		...
		if (! tSimpleActionReport.equals ("")) {
			gameManager.appendToGameActivity (tSimpleActionReport);
		}
	}
	...
	public boolean applyAction (Action aAction) {
		...
		tActionApplied = aAction.applyAction (gameManager);
